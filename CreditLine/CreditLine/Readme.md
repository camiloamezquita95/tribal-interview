...
WORKDIR /src
COPY ["CreditLine.csproj", "CreditLine/"]
RUN dotnet restore "CreditLine/CreditLine.csproj"

WORKDIR "/src/CreditLine"
COPY . .
RUN dotnet build "CreditLine.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "CreditLine.csproj" \
...
Execute unit tests
```
    cd "CreditLine/test/CreditLine.Tests"
    dotnet test
```

Deploy application
```
    cd "CreditLine/src/CreditLine"
